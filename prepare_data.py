...
def split_and_normalize_data(output_file, output_scaler_file, train_size):
    with h5py.File(output_file, 'a') as f:
        ...
        normalize_max = 0.95
        normalize_min = 0.05
        min_X_values = np.min(X[train_indices], axis=(0, 1)).astype(np.float32)
        max_X_values = np.max(X[train_indices], axis=(0, 1)).astype(np.float32)
        min_y_values = np.min(y[train_indices], axis=(0)).astype(np.float32)
        max_y_values = np.max(y[train_indices], axis=(0)).astype(np.float32)
        ...
        # Normalizza i dati
        X = (X - min_X_values) / (max_X_values - min_X_values) * (normalize_max - normalize_min) + normalize_min
        y = (y - min_y_values) / (max_y_values - min_y_values) * (normalize_max - normalize_min) + normalize_min
